...
test("unexpected 1 error - specify error code only", t => {
    ...
    const f = failures[0];
    t.true(f.line === 5);
    t.true(f.expected !== undefined && f.expected.code.join(",") === "TS0001,TS0002");
    t.true(f.actual !== undefined && f.actual.code === "TS2322");
});
...
test("unexpected success", t => {
    ...
    t.true(f1.line === 5);
    t.true(f1.expected !== undefined && f1.expected.code.join(",") === "TS2324,TS9999");
    t.true(f1.actual === undefined);

    t.true(f2.line === 10);
    t.true(f2.expected !== undefined && f2.expected.code.join(",") === "TS9999,TS2322");
    t.true(f2.actual === undefined);

    t.true(f3.line === 11);
    t.true(f3.expected !== undefined && f3.expected.code.join(",") === "TS2322");
