...
function formatError(error, title, titleColor, detailColor) {
    ...
    var text = error.message ? error.code + ": " + error.message : error.code;
 ...
 * For internal use
 */
function formatResultForCli(fileName, failures) {
    var ret = [];
    failures.forEach(function (failure) {
        ret.push(colors_1.colors.title(fileName + ":" + (failure.line + 1)));
        ret.push(formatError(failure.expected, "  expected: ", colors_1.colors.errorTitle, colors_1.colors.errorDetail));
        ret.push(formatError(failure.actual, "  but was:  ", colors_1.colors.errorTitle, colors_1.colors.errorDetail));
        ret.push("");
    });
    return ret.join("\n");
}
exports.formatResultForCli = formatResultForCli;
/**
 * Tester object
 */
var Tester = (function () {
